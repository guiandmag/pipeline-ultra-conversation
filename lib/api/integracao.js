...
NOTAS:
	...
	- Depois de usar uma função vinda do contexto, lembre-se sempre de LIMPAR O CONTEXTO, retirando a função "func";
	- Após a completa realização das funções salvarInfo, solicitarPrioridade e reabrirChamado, ou seja, depois
		da confirmação da API do SCCD e do chamado alterado, enciar uma mensagem de sucesso ao usuário;
	- Ao final da função idChamado, o app deve enviar uma pergunta vazia ao Watson com a variável "found" correta no contexto;

MODELO DO CONTEXTO:

	context: {
		id: código do chamado a ser procurado (e.g. RDS1234567, RDS 1234567 -> lembrar de tirar o espaço antes de procurar no SCCD),
		tipo: RDS, INC, RDM ou ATV (tipo do chamado),
		found: true/false (encontrou o chamado referido),
		chamado: {
			status: resolvido, encerrado, em atendimento, aguardando atendimento, pendente, cancelado, agendado ou aguardando info,
			solution: solução aplicada em chamados resolvidos/encerrados,
			pendency: pendencia do chamado,
			reason: motivo do cancelamento/agendamento do chamado,
			date: data de agendamento do chamado,
			info: informações solicitadas
		},
		motivo: motivo da reabertura do chamado inserido pelo usuário,
		info: informações solicitadas inseridas pelo usuário,
		func: idChamado (procurar chamado pelo código), novoChamado, salvarMotivo (para reabertura),
				aguardarConfirmReabertura (para reabertura), reabrirChamado (para reabertura), solicitarPrioridade,
				mudarStatusAtendimento (mudar o status de agendado para em atendimento), aguardarConfirmInfo (para adição de info),
				salvarInfo (adicionar informações solicitadas),
		alternar: chamado, status, faq, limbo (alternador de workspaces),
		reenviar: reenvia a mensagem (uso somente para alternar de workspaces)
	}
...
var APIParser = function(messageToUser, callback, partial) {
	...
	var funcao = messageToUser.context.func;
	...
	switch(funcao) {
		...
		case "idChamado":
			/*  Deve buscar o chamado através da API do service portal usando o ID passado no contexto (context.id).
				Retorna uma mensagem vazia para a API do Watson com a variável booleana "context.found" devidamente colocada.
			 */

			// MOCK
			var contexto = extend({}, messageToUser.context, {found: true, chamado: {status: "pendente", pendency: "Pendencia de teste"}, func: null}); // Sempre limpar a func
			return sendMessageToConversation({context: contexto}).then(function(msg) { // Isso vai enviar a mensagem vazia ao watson
				APIParser(msg, callback);
			})
		...
		case "aguardarConfirmReabertura":
		...
		case "solicitarPrioridade":

		break;

		case "mudarStatusAtendimento":

		break;

		case "aguardarConfirmInfo":

		break;

		case "salvarInfo":

		console.log("Salvar informações a seguir")
